    ...
    <b-row>
      <b-col md="3">
        <div v-show="!markerOrigin.position.lat">
          <b-alert show variant="info">1. Klik Pada Peta Untuk Memilih Asal Perjalanan
          </b-alert>
        </div>
      </b-col>
      <b-col md="3">
      ...
      <b-col md="3">
        <div class="mb-3" v-if="
                        input.parking_guarantor!==null && input.travel_model!==null && input.vehicle_availability!==null && input.transportation_mode!==null && !routes
                    ">
          <b-button class="shadow" block variant="success" @click="submit">Submit
          </b-button>
        </div>
      </b-col>
    </b-row>
        ...
        <b-row>
          <b-col cols="12">
            <b-card no-body class="shadow mb-3">
              <TravelMap ref="travelmap" class="travel-map" :apiKey="apiKey" :markerOrigin="markerOrigin" :markerDestination="markerDestination" :areaOrigin="origin.area" :areaDestination="destination.area" :input="input" @routes="routes = $event" @transportationModes="
      ...
      <b-col md="6">
        <b-card class="shadow mb-3" v-show="markerDestination.position.lat &&
          ...
          <div>
            ...
            <b-form-radio-group slot="bottom" id="travel_model" button-variant="outline-primary" v-model="input.travel_model" :options="options.travel_models" buttons stacked name="travel_model" class="btn-block"></b-form-radio-group>
          </div>
          <div>
            <label for="vehicle_availability">
              Ketersediaan Kendaraan bermotor
            </label>
            <b-form-radio-group slot="bottom" id="vehicle_availability" button-variant="outline-primary" v-model="input.vehicle_availability" :options="options.vehicle_availabilities" buttons stacked name="vehicle_availability" class="btn-block"></b-form-radio-group>
          </div>
          <div>
            <label for="transportation_mode">
              Moda Transportasi
            </label>
            <b-form-select plain id="transportation_mode" class="form-control-lg shadow-sm" :options="
                            [0].includes(input.travel_model)
                                ? options.transportation_modes.slice(2, 6)
                                : options.transportation_modes
                        " v-model="input.transportation_mode" :disabled="input.travel_model==null">
              <template slot="first">
        ...
        <div class="pb-3" v-show="surveyPreference.length >0
                    ">
          <b-card title="Pertanyaan Preferensi" class="shadow">
          ...
          <b-card title="Hasil Simulasi" class="shadow">
            ...
            <div>
              <div v-if="routes.mrt.path.length !== 0">
                ...
                <div>
                  <b-badge variant="primary" v-for="(v, i) in routes.mrt.stations" :key="`station-${i}`">
                    {{ v.name }}
                  </b-badge>
                </div>
                <div v-if="
