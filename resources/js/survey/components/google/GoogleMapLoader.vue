    ...
    methods: {
      ...
      initializeMap() {
        ...
        this.google.maps.event.addListener(this.map, 'click', function (event) {
          ...
          console.log("TCL: initializeMap -> event.latLng", event.latLng)
          // console.log(area)
          let latlng = new self.google.maps.LatLng(event.latLng.lat(), event.latLng.lng())
          axios.get(`geocoding?latlng=${event.latLng.lat()+","+event.latLng.lng()}`)
            .then((response) => {
      ...
      getPath(area) {
        ...
        for (let index = 0; index < coverage[area].length; index++) {
          element[index] = new this.google.maps.LatLng(coverage[area][index]['lat'], coverage[area][index]['lng'])
        }
