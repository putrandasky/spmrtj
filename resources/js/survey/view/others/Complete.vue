<template>
  <b-col lg="9" md="10" sm="12">
    ...
    <div class="text-center mb-5">
      <p class="display-2 font-weight-bold text-white text-shadow title-form">
        TERIMA KASIH
      </p>
    ...
    <b-row class="d-flex justify-content-center" v-if="!isLoading && !isSubmitted">
      <b-col lg="6">
        <b-card class="shadow text-center font-weight-bold">
          ...
          <b-button variant="success" block v-if="!isSubmitted" @click="inputDataModal = true">Isi data diri</b-button>
        </b-card>
      </b-col>
    </b-row>
    <b-modal v-if="!isSubmitted" :no-close-on-esc="true" :hide-header-close="true" :no-close-on-backdrop="true" title="Mohon isi data diri anda" v-model="inputDataModal" @ok="postData" ok-title="Kirim" cancel-title="Batal" ok-variant="primary">
      ...
      <b-form-group label="Apa harapan anda untuk MRT Jakarta" :invalid-feedback="errors.comment" :state="stateComment">
        <b-form-textarea id="harapan" v-model="input.comment" :state="stateComment" placeholder="Tuliskan harapan anda disini" rows="6" max-rows="6"></b-form-textarea>
      </b-form-group>
    </b-modal>
