  ...
  <div>
    ...
    <b-row v-if="step == 1">
      <question-slot>
        ...
        <template slot="text">* Asal perjalanan, biasanya rumah / tempat tinggal atau
          homebase trip berbasis tempat tinggal</template>
        <template slot="bottom">
          <auto-complete-place class="w-100 pt-0 d-none" placeholder="Ketik untuk cari..." @hit="addPlace($event, 'origin')" @onkeyup="input.travel_origin = null" readonly></auto-complete-place>
        </template>
      </question-slot>
      ...
      <b-button v-if="input.travel_origin" type="submit" block variant="success" class="mx-auto px-5 mt-3" @click="handleNextStep(2)">
        Lanjut
      </b-button>
    </b-row>
    <b-row v-if="step == 2">
      <question-slot>
        ...
        <template slot="text">*Perjalanan rutin untuk beraktivitas sehari- hari dalam
          kondisi normal : bekerja, sekolah, berdagang, dll</template>
        <template slot="bottom">
          <auto-complete-place class="w-100 pt-0 d-none" placeholder="Ketik untuk cari..." @hit="addPlace($event, 'destination')" @onkeyup="input.travel_destination = null" readonly></auto-complete-place>
        </template>
      </question-slot>
      ...
      <b-button v-if="input.travel_destination" type="submit" block variant="success" class="mx-auto px-5 mt-3" @click="step = 3">
        Lanjut
      </b-button>
    </b-row>
    ...
    <question-slot class="w-100  " v-if="step == 3">
      ...
      <template slot="bottom">
        <b-form-radio-group id="travel_purpose" stacked v-model="input.travel_purpose" :options="options.travel_purposes" button-variant="outline-primary" buttons name="travel_purpose" class="btn-block"></b-form-radio-group>
        <b-btn variant="success" class="shadow-sm" block @click="step = 4" v-if="input.travel_purpose">
          Lanjut
        </b-btn>
      </template>
    </question-slot>
    ...
    <question-slot class="w-100  " v-if="step == 4">
      ...
      <template slot="bottom">
        <b-form-radio-group slot="bottom" id="travel_model" stacked button-variant="outline-primary" v-model="input.travel_model" :options="options.travel_model" buttons name="travel_model" class="btn-block"></b-form-radio-group>
        <b-button block type="submit" variant="success" class="mx-auto px-5" @click="step = 5" v-if="input.travel_model !== null">Lanjut</b-button>
      </template>
    </question-slot>
    <div v-if="step == 5">
      <question-slot>
        ...
        <template slot="bottom">
          ...
          <range-slider id="trip_frequency" class="p-0" v-model.number="input.trip_frequency" min="1" max="7" step="1" :disabled="submitting"></range-slider>
          <div>
            <small>
      ...
      <question-slot class="border-top pt-3 mt-3" v-if="[0, 2].includes(input.travel_model)">
        <hr />
        <template slot="above">
        ...
        <template slot="bottom">
          <b-form-radio-group slot="bottom" id="is_pay_for_park" button-variant="outline-primary" v-model="input.is_pay_for_park" :options="options.is_pay_for_park" buttons name="is_pay_for_park" class="btn-block" @change="isChangePayForPark"></b-form-radio-group>
        </template>
      </question-slot>
      <question-slot class="border-top pt-3 mt-3" v-if="input.is_pay_for_park == 1">
        <hr />
        <template slot="above">
        ...
        <template slot="bottom">
          ...
          <range-slider id="parking_cost" class="p-0" v-model.number="input.parking_cost" min="0" max="50000" step="1000" :disabled="submitting"></range-slider>
          <div>
            <small>
              Nb : Geser untuk memilih
            </small>
      ...
      <question-slot class="border-top pt-3 mt-3" v-if="input.is_pay_for_park == 1">
        ...
        <template slot="bottom">
          <b-form-radio-group slot="bottom" id="parking_guarantor" button-variant="outline-primary" v-model="input.parking_guarantor" :options="options.parking_guarantors" buttons name="parking_guarantor" class="btn-block"></b-form-radio-group>
        </template>
      </question-slot>
      ...
      <question-slot class="border-top pt-3 mt-3" v-if="[0, 2].includes(input.travel_model)">
        ...
        <template slot="bottom">
          <b-form-radio-group slot="bottom" id="parking_type" stacked button-variant="outline-primary" v-model="input.parking_type" :options="options.parking_types" buttons name="parking_type" class="btn-block"></b-form-radio-group>
        </template>
      </question-slot>

      <b-button block type="submit" variant="success" class="mx-auto px-5 mt-3" @click="step = 6" v-if="
    ...
    <div v-if="step == 6">
      <b-row class="px-3" v-for="(v, i) in input.travel_detail" :key="i" v-show="!isLoading && input.travel_detail.length == i + 1">
        ...
        <b-form-select plain id="transportation_mode" class="form-control-lg shadow-sm" :options="getTransportationModeByTravelModel" v-model="v.transportation_mode" :disabled="submitting" @change="handleInputTransportationMode($event, i)">
          <template slot="first">
        ...
        <div class="w-100 mt-3" v-if="v.transportation_mode">
          <div v-if="
            ...
            <range-slider id="waiting_duration" class="p-0" v-model.number="v.waiting_duration" min="0" max="30" step="1" :disabled="submitting"></range-slider>
          </div>
          <div>
            <hr />
            ...
            <range-slider id="travel_duration" class="p-0" v-model.number="v.travel_duration" min="0" :max="
                            ...
                            " :step="
                                hasLessDuration.indexOf(
                                    ...
                                    ? 5
                                    : 1
                            " :disabled="submitting"></range-slider>
          </div>
          <hr />
          <div v-if="
            ...
            <range-slider id="travel_cost" class="p-0" v-model.number="v.travel_cost" min="0" :max="
        ...
        <b-button class="mx-auto px-5 mt-2" variant="secondary" @click="back(i)" :disabled="submitting" v-if="
                        !(
                    ...
                    ">Kembali</b-button>
        <div class="w-100 text-center mt-2" v-if="
                        v.transportation_mode > 0 &&
                            (notRequiredWaitingTime.indexOf(
                                v.transportation_mode
                            ) == -1
                            ...
                            -1
                                ...
                                : true)
                    ">
          <b-button variant="secondary" @click="back(i)" :disabled="submitting" v-if="i > 0">Kembali</b-button>
          <b-button variant="info" @click="addTravelDetail" :disabled="submitting" v-if="input.travel_model > 0">Moda berikutnya</b-button>
          <b-button variant="success" @click="showConfirmDialog" :disabled="submitting">
            Kirim
            <b-spinner small type="grow" v-show="submitting"></b-spinner>
          </b-button>
        </div>
      </b-row>
    </div>
    <b-modal v-model="modalWelcome" :no-close-on-esc="true" :hide-header-close="true" :no-close-on-backdrop="true" :hide-header="false" ok-only hide-footer centered>
      <template v-slot:modal-header>
        <div class="d-flex justify-content-start align-items-center w-100">
          <span>
            <lottie :options="defaultOptions" v-on:animCreated="handleAnimation" :height="30" :width="30" />
          </span>
      ...
      <p class="mb-3 text-justify">
        ...
        <strong>berawal dari rumah (tempat tinggal/homebase trip berbasis
          tempat tinggal)
        </strong>
        menuju
        <strong>
      ...
      <b-btn variant="primary" class="float-right" @click="modalWelcome = false">Saya Mengerti</b-btn>
    </b-modal>
  </div>
</template>
<script>
      ...
      return {
        ...
        options: {
          ...
          travel_model: [{
            ...
            {
              text: "Mobil/Motor dan Angkutan Umum (kombinasi keduanya)",
              value: 2
            }
          ]
        }
